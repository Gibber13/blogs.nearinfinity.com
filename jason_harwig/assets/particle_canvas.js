...
(function() {
  ...
  window.Emitter = Class.create({
    initialize: function(input, style) {
      this.input = $(input);        
      this.input.insertCanvas({before:['150', '150']});
      this.alignCanvas();
      this.options = this.input.getAttribute('options').evalJSON();
      this.particles = [];
      ...
      this.input.observe('click', this.start.bind(this));
    },
    ...
    alignCanvas: function() {
      this.inputOffset = this.input.positionedOffset();
      this.inputDim = this.input.getDimensions();
      this.canvas = this.input.previous('canvas');    
      this.canvas.setStyle('position:absolute; top:' + (this.inputOffset[1]-150) + 'px; left: ' + (this.inputOffset[0]-(75-this.inputDim.width/2)) + 'px;');    
    },

    prepareParticles: function() {
        var inst = this,
          halfInputW = this.inputDim.width/2 * 0.6,
